  ...
  cimg_forXY(img,x,y){           // recorro la imagen
    if (img(x,y)==blanco){       // si es blanco
      ...
      if (vecinos.empty()) {     // si no tengo vecinos etiquetados debo generar nueva etiqueta
        ...
        for (size_t i=0; i<vecinos.size()-1; i++)  // controlo la lista de etiquetas vecinas
...
///****************************************
CImg<double> hough(CImg<double> img, bool inverse, double &tita,  int *v) {
...
int cont = 0;
  ...
  if (!inverse){ 
    ...
    cimg_forXY(img,y,x){
      if (img(y,x) > 0.5)
        for (size_t t=0; t<M; t++) { //calculo rho variando theta en todo el eje, con x e y fijo
