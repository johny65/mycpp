...
static void doit_and_select(guint group_id, guint key_id)
{
	...
	if (sci){
		...
		//ubicarme en el (
		int act = sci_get_current_position(sci);
		int num = sci_get_current_line(sci);
		int nn = sci_get_line_length(sci, num);
		int p = act - sci_get_position_from_line(sci, num);
		gchar *l = sci_get_line(sci, num);
		gboolean found = FALSE;
		int k, c=0;
		for (k = p; k < nn; k++){
			if (l[k] == '('){
				sci_set_current_position(sci, act+c+1, TRUE);
				found = TRUE;
				break;
			}
			c++;
		...
		//seleccionar
		//int current = sci_get_current_position(sci);
		//
		//int linnum = sci_get_current_line(sci);
		//int n = sci_get_line_length(sci, linnum);
		//gchar *lin = sci_get_line(sci, linnum);
		//printf("%s\n", lin);
//
		//int pos = current - sci_get_position_from_line(sci, linnum);
		//
		//int i, a=-1, b=-1, c = 0;
		//printf("pos actual: %c\n", lin[pos]);
		//for (i = pos; i >= 0 ; --i){
